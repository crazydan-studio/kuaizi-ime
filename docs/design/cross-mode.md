...
- 交叉线从中心向外辐射，形成**辐射轴**，再沿轴的辐射方向，依次放置分配到侧边上的字符，
  且最多可放置四个字符
- 经过 `(分区, 分区侧边, 轴序号)` 三个坐标的布局，在 X 型输入中，可以最多放置
  `4（个分区）x 2（条分区侧边）x 4（个字符）= 32` 个字符
...
**第一步**：选中目标输入字符所在分区的侧边。
即，从中心出发滑入字符 `B` 所在的 `0#` 分区，接着，沿逆时针方向滑入 `3#`
分区，再滑回到中心：

![](./cross-mode/20-lantin-input-active-zone.png)

> 注：若是最终依然从 `0#` 分区滑回中心，则表示取消对该分区字符的选择，
> 可重新选择其他输入字符所在的分区。
...
**第二步**：选择目标输入字符。即，继续从中心滑入布局展开后字符 `B` 所在的 `1#` 分区。
此时，由于每个字符均独占一个分区，而在分区内滑行又有逆时针和顺时针两个方向可以选择，
这正好可以用于区分字符的大小写两种形式。因此，这里暂且规定，若从 `1#`
分区逆时针方向滑入 `0#` 分区并滑回中心，则输入大写字符 `B`，
而若沿顺时针方向从 `1#` 分区滑入 `2#` 分区再滑回中心，则输入小写字符
...
> 注：若是最终依然从 `1#` 分区滑回中心，则表示取消对字符 `B` 的选择，可重新选择其他字符。

至此，通过两步滑行，便可唯一确定大写或小写形式的字符 `B`。

在完成字符的输入后，输入面板将自动恢复到初始布局。因此，无需中断滑行，
可继续从中心滑入目标输入字符所在的分区（比如 `1#`），并按照上述过程输入其他字符即可：
...
- 每个字符需要且仅需要两次滑行便可完成输入，且其定位坐标为 `(分区, 分区侧边, 分区, 分区侧边)`
- 两次滑行均需以固定的位置作为起止点，故而，可将该起止点称为**轨迹原点**，
  在轨迹原点半径范围内的滑入滑出均视为滑行的有效起止点
- 每次滑行形成的轨迹都是一个椭圆形，并且以十字型输入的辐射轴为椭圆的长轴
- 两个椭圆形会以水平、垂直或重叠的形式组成字符的输入轨迹
- 通过两个椭圆轨迹的不同组合，以及在椭圆上的滑行方向（顺时针或逆时针），便可唯一确定全部输入字符
- 每个字符的轨迹都是相似的，滑行次数也是相同的，故其相互之间没有优劣之分，
  因此，不需要按照使用频率等因素来确定字符的布局，直接按字母顺序沿顺时针方向依次放置即可

若是在输入轨迹上加上运动方向，则可以看到，输入轨迹中的两个椭圆是具有**方向性**的，
且从轨迹原点出发，仅有顺时针和逆时针两个方向。因此，我们可以用以下两个箭头代表不同方向的椭圆：

- 背离轨迹原点的箭头：其箭头从原点指向外部，用以表示沿逆时针方向滑行的椭圆
- 指向轨迹原点的箭头：其箭头从外部指向原点，用以表示沿顺时针方向滑行的椭圆

这样，便可以得到如下图所示的输入轨迹与箭头的映射关系：
...
- 椭圆轨迹与坐标轴相交的点便为其长轴的端点，其包含轨迹原点
- 椭圆轨迹长轴的端点便是其对应向量的起止点
- 分析椭圆轨迹上点的运动方向，便可确定椭圆方向，进而确定向量方向，最终完成转换

沿横纵坐标可以定义 `4` 组向量，每组向量有 `2` 个向量，而每个向量可以有 `4` 个方向，
所以，通过向量可以表示 `4 * 2 * 4 = 32` 个输入轨迹，
正好与十字型输入可布局的最大字符数相等，进一步证明二者的转换是等效的。

在确定了从椭圆轨迹到向量的转化规则后，字符输入便可摆脱布局空间的限制，可以以任意点为轨迹原点，
只要滑行轨迹能够正确转换为两个水平或垂直方向上的向量即可。

下图表示的是从非中心位置开始的字符输入轨迹，其对应的是小写字符 `f` 的输入：

![](./cross-mode/50-lantin-blind-hit.png)

> 注：对于盲打的输入支持，可以详见章节[《盲打输入模式》](#盲打输入模式)。

从以上分析不难发现，十字型输入的轨迹图形更便于做程序分析，其仅涉及垂直和水平两个方向，而
X 型输入则需要做 45° 转换处理。更重要的是，围绕十字形的轴线画椭圆仅需较小幅度的分区跨越即可，
也就是，椭圆轨迹的短轴可以很短，但 X 型输入滑行的椭圆轨迹的短轴则相对要更长一些，
因此，十字型输入会比 X 型输入更方便、更省力、更准确。
...
- 可在四个字符分区各放置一到两个功能按键
- 如何自然地切换到数字、算术等输入键盘？
  - 在中心画圈，类似于拨动旋钮，根据画圈的数量来循环切换不同的输入
    - 模拟阻尼滚轮
