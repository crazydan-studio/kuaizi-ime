...
十字型输入依然是通过四条轴将输入面板均分为四个区域，只是四条轴是水平和垂直的，构成一个「十」字，
而不像 X 型输入是做了 45 度旋转，形成的是一个 `X`。
...
**第二步**：继续从中心原点滑入 `1#` 区域，此时，在第二维度的字符展开后，每个字符均独占一个区域，
而字符具有大小写两种形式，正好可通过逆时针和顺时针两个滑行方向来确定其大小写形式。
