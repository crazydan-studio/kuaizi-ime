...
- 交叉线从中心向外辐射，形成**辐射轴**，再沿轴的辐射方向，依次放置分配到侧边上的字符，
  且最多可放置四个字符
- 经过以上三个维度的布局，在 X 型输入中，可以最多放置 `4（个分区）x 2（条分区边）x 4（个字符）= 32`
  个字符

而其输入逻辑如下（以上图中的字符 `O` 的输入为例，并将四个分区按位置命名为上、下、左、右分区）：

- 从中心原点出发，滑入字符 `O` 所在的`上分区`。这表示激活对该分区字符的选择
- 由于 `O` 在分区的左侧边上，因此，需沿逆时针方向，滑入`左分区`。这表示选择`上分区`左侧边的字符
- 继续保持滑行方向不变，滑入`下分区`，并直接滑回中心原点。这表示选择沿轴辐射方向上的第二个字符，
  也就是当前所要输入的字符 `O`
- 其中，手指滑过的分区数量，对应的便是字符在辐射轴上的位置
- 并且，只有滑回中心原点，才表示一个字符输入过程的结束
...
- 充分利用分区、分区侧边、轴辐射方向三个**空间维度**来**布局字符**，在保证输入字符全部可见的前提下，
  使得操作空间更大
...
因为，通过画圈时经过的分区数量来定位相应位置的字符，其形成的滑行轨迹都只是不同角度的圆弧，
...
**第一步**：选中目标输入字符所在分区的侧边。
即，从中心原点出发滑入字符 `B` 所在的 `0#` 分区，接着，沿逆时针方向滑入 `3#`
分区，再滑回到中心原点：

![](./cross-mode/20-lantin-input-active-zone.png)

> 注：若是最终依然从 `0#` 分区滑回中心原点，则表示取消对该分区字符的选择，
> 可重新选择其他输入字符所在的分区。

在选中分区侧边后，十字型输入将自动采用**动态布局**机制，
将分区侧边上的（最多）四个字符重新布局到四个分区中。
也就是，对侧边上排列的字符做**布局展开**：
...
**第二步**：选择目标输入字符。即，继续从中心原点滑入布局展开后字符 `B` 所在的 `1#` 分区。
此时，由于每个字符均独占一个分区，而在分区内滑行又有逆时针和顺时针两个方向可以选择，
这正好可以用于区分字符的大小写两种形式。因此，这里暂且规定，若从 `1#`
分区逆时针方向滑入 `0#` 分区并滑回中心原点，则输入大写字符 `B`，
而若沿顺时针方向从 `1#` 分区滑入 `2#` 分区再滑回中心原点，则输入小写字符 `b`：
...
> 注：若是最终依然从 `1#` 分区滑回中心原点，则表示取消对字符 `B` 的选择，可重新选择其他字符。

至此，通过两步滑行，便可唯一确定大写或小写形式的字符 `B`。

在完成字符的输入后，输入面板将自动恢复到初始布局。因此，无需中断滑行，
可继续从中心原点滑入目标输入字符所在的分区（比如 `1#`），并按照上述过程输入其他字符即可：
...
- 每个字符的滑行轨迹都是由两个 `O` 以水平、垂直或重叠的方式组成
- 通过两个 `O` 的组合形式，以及轨迹滑行的运动方向，便可唯一确定全部输入字符
- 每个字符的轨迹是相似的，滑行次数是相同的，故其相互之间没有优先级关系，都是平等的，
  因此，不需要按照使用频率等因素来确定字符的布局，直接按字母顺序沿顺时针方向放置即可

之所以采用「十」字形输入，而非「X」形，
一个是原因，在十字形上绘制的滑行轨迹会被限定在垂直和水平方向上，更便于程序分析，
而最重要的原因是，在水平或垂直方向上滑行，会比在倾斜方向上滑行，更加方便和准确。
...
- 协调手指做出响应，从键盘输入分区的任意位置开始，沿着大脑确定出的滑行轨迹和运动方向，迅速滑出字符的输入轨迹
...
虽然，十字型输入形成的字符输入轨迹是由两个 `O` 组成，并且具有方向性，
但在滑行轨迹识别上，我们可以将两个 `O` 的绘制简化为方向向量，再对两个向量组成的结果进行分析即可。

很容易发现，每个 `O` 仅有逆时针和顺时针两种滑行方式，因此，可以约定：

- 沿逆时针方向滑行的 `O` 用箭头背离中心原点的向量表示
- 沿顺时针方向滑行的 `O` 用箭头指向中心原点的向量表示

这样，便可以得到如下图所示的轨迹与向量的映射关系（暂且用箭头表示向量）：

![](./cross-mode/40-input-trace-to-vector.png)

经过向量简化后，轨迹识别将转换为以下处理过程：

- 确定两个 `O` 的交叉点，并以交叉点作为轨迹的原点
- 找到 `O` 向外的顶点，即，在其周围绘制一个矩形，其与矩形的边刚好相交的点
- 根据轨迹运动方向，确定其为顺时针还是逆时针方向，进而得到从 `O` 的顶点到原点方向上的向量
- 将得到的真实的滑行向量与字符理想的滑行向量向比对，若二者相同，则表示将输入该字符

所以，只需要能够识别出轨迹，便可直接打出对应的字符，而不必等待面板对动态布局的视图刷新，
以至于面板还没开始刷新视图，输入便已经完成了。
如下图所示，动态布局还未在视图上更新，字符 `f` 便已输入完成了：
...
- 可在四个字符分区各放置一到两个功能按键
- 如何自然地切换到数字、算术等输入键盘？
  - 在中心原点画圈，类似于拨动旋钮，根据画圈的数量来循环切换不同的输入
