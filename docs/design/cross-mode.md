...
之所以采用「十」字形输入，而非「X」形，
一个原因是，在十字形上绘制的滑行轨迹会被限定在垂直和水平方向上，更便于程序分析，
...
但在滑行轨迹识别上，我们可以将两个 `O` 的绘制简化为两个向量，再对两个向量组成的结果进行分析即可。
...
- 将得到的真实的滑行向量与字符理想的滑行向量相比对，若二者相同，则表示需输入该字符

所以，只需要能够识别出轨迹，便可直接打出对应的字符，而不必等待面板对动态布局的视图刷新。
如下图所示，动态布局还未在视图上开始刷新，字符 `f` 便已输入完成了：
