...
依赖: python-svgpathtools, pillow, numpy, opencv, scikit-learn
安装: sudo pacman -S python-svgpathtools python-pillow python-numpy python-opencv python-scikit-learn
"""
...
def load_features_from_db(db_path, n_coeffs=None, canvas_size=None, sample_points=None):
    ...
    # 读取所有特征
    cursor.execute("select id, feature from meta_zi_stroke_path_feature")
...
def main():
    ...
    if not force_recompute:
        ...
        if features is not None:
            print(f"已加载已缓存的笔画路径特征 {len(features)} 个")
    ...
    # 聚类
    labels = cluster_paths(features, eps=eps, min_samples=2)
